<?php 
    ...
    if(isset($_SESSION['admUser']) && !empty($_SESSION['admUser'])):
        ...
        if (isset($_GET['busca']) && !empty($_GET['busca'])) {
            ...
            $sql = "SELECT * FROM usuarios WHERE nome LIKE :termo OR email LIKE :termo OR codigo = :codigo";
...
<body>
    <div class="header" id="header">
        ...
        <div class="buscar">
            <form method="GET" action="lista_de_usu.php">
                <label for="busca">Buscar:</label>
                <input type="text" id="busca" name="busca" oninput="buscarUsuarios()">
        ...
        ?>
             <table summary="Tabela de pedidos" border="1">
                ...
                foreach ($result as $row) {
                    ...
                    echo "<td>***************</td>";
                    echo "<td><button class='btn-clear'><a id='link' href='editar_usuario.php?codigo={$row['codigo']}'><i class='bi bi-pencil-square'></i></a></button></td>";//botão para editar um usuário
                    echo "<td><button class='btn-clear'><a id='link' href='processar_exclusao.php?codigo={$row['codigo']}'><i class='bi bi-trash'></i></a></button></td>";//botão para deletar um usuário
                    echo "</tr>";
                }
                ?>
 ...
 <script>
        function buscarUsuarios() {
            ...
            // Faz uma solicitação AJAX para buscar usuários
            var xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    // Atualiza a tabela com os resultados
                    document.getElementById('tabela-usuarios').innerHTML = xhr.responseText;
                }
            };
            xhr.open('GET', 'lista_de_usu.php?busca=' + termo, true);
            xhr.send();
        }
    </script>
